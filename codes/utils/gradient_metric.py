#This codes covers gradient level information so that user can get a information on gradients
import numpy as np

class grad_information(object):
    @staticmethod
    def get_grads(model) :#changes with choosen models toDo:change this to gnerelize ones
        list_=[]
        def select_obj2(key):
            splitted=block.split('.')
            key=splitted[0]
            if key in ['conv_blocks']:
                key2 = int(splitted[1])
                key3 = splitted[2]
                return model.conv_blocks[key2].conv
            elif key =='fc1':return model.fc1
            elif key == 'fc2':return model.fc2
            elif key == 'fc1_p':return model.fc1_p
            else :return None
        ...
        for block in model.state_dict().keys():
            ...
            if obj is not None:list_.append(obj.weight.grad.numpy().flatten())
        return list_
