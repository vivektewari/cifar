...
class GRUCell(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        # resetgate = F.sigmoid(i_r) + F.sigmoid(h_r)
        # inputgate = F.sigmoid(i_i) + F.sigmoid(h_i)
...
class GRUModel(nn.Module):# for 1 rc value only and different fc for each seq
    ...
    def forward(self, x):
        ...
        return out_val
class GRUModel_ext(nn.Module):# for 1 rc value only and different fc for each seq
    def __init__(self, input_dim, hidden_dim, output_dim,seq_count, rc_seq):#ori: layer_dim,
        super(GRUModel_ext, self).__init__()
        # Hidden dimensions
        self.hidden_dim = hidden_dim
        self.Gru=GRUModel(input_dim, hidden_dim, output_dim,seq_count,rc_seq)
        self.activation_l = torch.nn.LeakyReLU()
        self.fc1s = nn.ModuleList()
        ...
        output_dim=16
        for i in range(2):
            self.fc1s.append(nn.Linear(input_dim, output_dim) )
            self.fc2s.append(nn.Linear(input_dim, output_dim))
            input_dim = 16
            output_dim = 1
        self.reset_parameters()
    def reset_parameters(self):
        std = 1.0 / math.sqrt(16)
        for w in self.parameters():
            w.data.uniform_(-std, std)
    def forward(self, x):
        solonoid=x[0]
        time_step=x[1]
        for i in range(2):
            solonoid=self.activation_l(self.fc1s[i](solonoid))
            time_step = self.activation_l(self.fc2s[i](time_step))
        x=solonoid*time_step
        x=self.Gru(x)
        return x
